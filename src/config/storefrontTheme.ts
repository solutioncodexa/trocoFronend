...
export type FontPairKey = 'display_sans' | 'editorial_serif' | 'modern_mono';
export type RadiusPresetKey = 'sharp' | 'soft' | 'round';
...
export function storefrontChipClass(themeKey: StoreThemeKey, radius: RadiusPresetKey): string {
  ...
  if (themeKey === 'elegant' || radius === 'round') return 'rounded-full';
...
export function storefrontProductImageClass(themeKey: StoreThemeKey, radius: RadiusPresetKey): string {
  const r = radius === 'sharp' || themeKey === 'bold' || themeKey === 'minimal' || themeKey === 'elegant'
    ? 'rounded-none'
    : radius === 'round'
      ? 'rounded-3xl'
      : 'rounded-2xl';
...
export function storefrontProductCardShell(
  ...
  cardStyle: 'elevated' | 'bordered' | 'flat' | 'minimal' = 'elevated',
): string {
