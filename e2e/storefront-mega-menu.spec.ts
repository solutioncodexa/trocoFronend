...
test.describe('Mega menu vitrine', () => {
  ...
  test('admin sections globales charge le mega-menu', async ({ page }) => {
    ...
    await expect(page.getByRole('tab', { name: /Mega menu/i })).toBeVisible({ timeout: 15_000 });
    await page.getByRole('tab', { name: /Mega menu/i }).click();
    await expect(page.getByText(/Remplace la navigation|sous-liens|mega-menu/i).first()).toBeVisible();
  });
});
